...
ggplot(data = hate_crimes, aes(x = hate_crimes, y = trump_support)) +
  geom_point() +
  scale_x_log10() +
  labs(x = "Number of Hate Crimes" , y = "Level of Trump Support", title = "Trump Support Per Number of Hate Crimes")
...
-   I would expect the relationship to be slightly positive.
:::
...
-   The intercept of this data is the mean amount of hate crimes in areas with high trump support.
:::
...
-   The model estimates 0.46 hate crimes per 100,000 people in states with "low" trump support.

```{r}
# Type your code and comments inside the code chunk
hate_crimes_low <- 0.191 + 0.269
hate_crimes_low
```
:::
...
-   The model estimates that states with lower trump support have more frequent hate crimes.
:::
...
```{r, label = "delta"}
# Type your code and comments inside the code chunk
hate_crimes_medium <- 0.191 + 0.031
hate_crimes_high <- 0.191
```

-   Hate crimes in "medium" trump support states are 0.031 greater per 100,000 than in high trump support states.
:::
...
-   The values are the same as the estimates $\hat{y}$ values
:::
...
$\hat{y}_{high} = 0.191 + 0.2691111 \times 0 + 0.0314 \times 0$
...
-   Oregon, Washington, Massachisetts, Minesota, and Maine are the states with the highest rate of hate crimes. The level of trump support in all of these places is low. 
:::
...
ggplot(data = hate_crimes, aes(x = unemployment , y = hate_crimes)) +
  geom_boxplot() +
  labs(x = "Unemployment Level", y = "Hate Crimes per 100,000", title = "Hate Crimes per 100k per Unemployment Level") 
...
::: {.callout-important icon="false" collapse="false" title="Problem 14 Answers"}
 $$\hat{y} = 0.289 + 0.031 \times \text{unemployment level}$$
:::
...
-   The intercept represents the mean number of hate crimes per 100,000 for states with high employment.
:::
...
```{r}
# Type your code and comments inside the code chunk
hc_low_unemployment <- 0.289 + 0.031
hc_low_unemployment
```

-   The model estimates 0.32 hate crimes per 100,000 people in states with low unemployment.
:::
...
# Type your code and comments inside the code chunk
job_mod |> 
  predict(newdata = data.frame(unemployment = c("low", "high"))) 


```

-   $\hat{y}_{low} = 0.320087$

-   $\hat{y}_{high} = 0.288750$
:::
...
-   The residuals are calculated by subtracting the values from hate_crimes_hat from hate_crimes
:::
...
-   There are more hate crimes in areas with higher median household incomes. Areas with low median household incomes have 0.16041 less hate crimes per 100,000 people.
:::
...
-   The model under predicted the hate_crimes level in maine.
:::
